...
- **data**: Esta pasta armazena os dados que serão processados pelo pipeline. Ela inclui tanto os dados de entrada quanto os resultados de saída. Os dados de entrada serão posteriormente armazenados em um Bucket no Amazon S3.
...
- **sh**: Esta pasta contém scripts em Shell que padronizam diversas tarefas de desenvolvimento (como adicionar novas pacotes no Python)
...
- **external_libs**: Subpasta que guarda as bibliotecas externas necessárias para os jobs do Glue. É garantido que cada job disponivel nesse repositório irá requerer pelo menos uma biblioteca encontrada aqui.
...
1. Crie um novo Bucket no S3. Ele será usado para armazenar os dados de entrada, dados das tabelas do Glue Catalog, dados de arquivos stage, código-fonte dos JOB's e consultas do Athena. Nesse exemplo, considere que o nome do Bucket é `repositorio-implementacao-pyspark`.

2. Crie as seguintes pastas no Bucket criado na etapa anterior:
    - `databases`: Pasta que armazena os databases disponiveis no Glue Catalog.
...
3. Faça upload de todos arquivos disponiveis na pasta do repositório `data` para a pasta `input_files` do Bucket. 

4. Abra o Athena e, quando for solicitado, informe o diretório da pasta `athena` do seu Bucket para que consultas sejam armazenadas (por exemplo: `s3://repositorio-implementacao-pyspark/athena`). Execute as declarações SQL do arquivo `db/athena_create_tables.sql` disponivel no repositório para criar as tabelas e o banco de dados no Glue Catalog via Athena. Lembre-se que cada declaração SQL deve ser executada de forma individual, por tanto, execute uma declaração por vez.

5. Agora criaremos uma ROLE no IAM para possibilitar a execucação dos JOB's. Acesse o IAM via Console da AWS
    - ![image](https://github.com/Guisilcol/Exemplo-de-implementacao-PySpark/assets/40049979/2aef9915-d765-4126-98da-7b681058ee48)

6. No menu lateral, clique em "Roles"
...
8. Siga as seguintes configurações para a criação da Role:
...
1. Certifique-se de que todos pré-requisitos estão instalados e funcionais na máquina, assim como o repositório clonado.

2. Editar o arquivo `start_dev_container.sh` (caso seu sistema operacional seja Windows, edite o arquivo start_dev_container.bat) e substituir o valor `DIRETORIO_AWS` pelo caminho do diretório onde estão as credenciais da AWS (pasta .aws)

3. Executar o script alterado na etapa anterior. Esse script irá subir um container com o ambiente de desenvolvimento configurado. Lembre-se de executar ele usando seu terminal estando na raiz do projeto.

4. Após a execução do script, o ambiente de desenvolvimento estará pronto para ser utilizado e você poderá acessá-lo através do VS Code usando a extensão 'Dev Container'. Na aba 'Remote Explorer', procure pelo container 'amazon/aws-glue-libs:glue_libs_4.0.0_image_01' e clique em 'Attach in Current Window'.
    ...
    - ![image](https://github.com/Guisilcol/Exemplo-de-implementacao-PySpark/assets/40049979/2e56ddb7-fc29-4813-9166-f2e03ab2ddfd)

5. Uma vez no ambiente de desenvolvimento, certifique-se de instalar suas extensões preferidas. Recomendo A extensao 'Python' e 'Jupyter' da Microsoft. 

6. Para testar o ambiente, crie um novo arquivo chamado `teste.py` na pasta `src` e insira o seguinte código:
...
7. Certifique-se que está usando a versão do Python 3.10.2 disponivel no container. Execute o código acima utilizando a opção 'Run Below' na primeira célula disponivel. Se tudo estiver configurado corretamente, você verá os resultados do código no terminal Jupyter que automaticamente será aberto.
    - ![image](https://github.com/Guisilcol/Exemplo-de-implementacao-PySpark/assets/40049979/f725f2de-4a45-47d0-ba64-79d6289ded6e)


8. Com o ambiente de desenvolvimento e AWS configurados, devemos alterar as variáveis de ambiente do projeto. Copie o arquivo `.env.example` para um arquivo chamado `.env` e preencha as variáveis de ambiente com as informações corretas, como segue o exemplo a seguir: 
...
9. Com as variáveis de ambiente configuradas, você pode começar a desenvolver os JOB's do Glue.
...
5. Renomeie o seu JOB clicando na "caneta" ao lado do nome dele
6. Clique a aba "Job Details" e configure de acordo com os prints a seguir. Certifique-se que o script se encontra na pasta de "src_code" criado no S3 anteriormente:
...
8. Uma vez configurado, certifique-se que todos os arquivos de origem estão disponibilizados no S3. Para testar o JOB, clique em "Run" no canto superior direito da tela. Para acompanhar a sua execução, clique na aba "Runs". Caso o JOB e o ambiente tenham sido configurados corretamente, o status será "Succeeded" 
